...
def trainSoftModel(test=False):
    # load training data
    X_train = np.array(pd.read_csv('./data/mnist_images_train.csv', header=None)).astype('float64')
    Y_train = np.array(pd.read_csv('./data/mnist_labels_train.csv', header=None))
    ...
    np.savetxt('./data/optimized_thetas.csv', np.array(thetas), delimiter=',')
    pass

def testSoftModel():
    X_test = np.array(pd.read_csv('./data/mnist_images_test.csv', header=None))#.astype('float64')
    Y_test = np.array(pd.read_csv('./data/mnist_labels_test.csv', header=None))
    ...
    Thetas = np.array(pd.read_csv('./data/optimized_thetas.txt', header=None)).astype('float64')
    Thetas = Thetas.reshape(10, -1)

    h = soft.h(Thetas, X_test)
    predictions = h.argmax(axis=1) + 1 # +1 for the 0-->10 positioning
    #for i in range(20):
